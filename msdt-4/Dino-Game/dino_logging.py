...
# настройка логирования по "Авторскому рецепту" 
default_extra = {
...
class CustomAdapter(logging.LoggerAdapter):
    def process(self, msg, kwargs):
        # Получаем словарь extra, переданный при вызове лога
        extra_from_call = kwargs.pop('extra', {})
        # Создаём итоговый extra, приоритет: из вызова > self.extra > default_extra
        final_extra = {}
        for key in ['user_id', 'key_id', 'ext_params']:
            final_extra[key] = extra_from_call.get(key, self.extra.get(key, default_extra.get(key, '')))
        # Вкладываем обработанный extra обратно
        kwargs['extra'] = final_extra
        return msg, kwargs


logger = logging.getLogger('DinoGame')
...
logger = CustomAdapter(logger, default_extra)

# Логируем старт скрипта
logger.info('SCRIPT_START', extra={'key_id': 0, 'ext_params': json.dumps({'game': 'Dino Run'})})
...
try:
    ...
    logger.info('SOUND_LOAD_SUCCESS', extra={'key_id': 10, 'ext_params': json.dumps({'sound': 'jump.wav'})})
except Exception as e:
    logger.error('SOUND_LOAD_FAILED', extra={'key_id': 500, 'ext_params': json.dumps({'sound': 'jump.wav', 'error': str(e)})})
    jump_sound = None

try:
    die_sound = pygame.mixer.Sound('resources/die.wav')
    logger.info('SOUND_LOAD_SUCCESS', extra={'key_id': 10, 'ext_params': json.dumps({'sound': 'die.wav'})})
except Exception as e:
    logger.error('SOUND_LOAD_FAILED', extra={'key_id': 500, 'ext_params': json.dumps({'sound': 'die.wav', 'error': str(e)})})
    die_sound = None

try:
    checkPoint_sound = pygame.mixer.Sound('resources/checkPoint.wav')
    logger.info('SOUND_LOAD_SUCCESS', extra={'key_id': 10, 'ext_params': json.dumps({'sound': 'checkPoint.wav'})})
except Exception as e:
    logger.error('SOUND_LOAD_FAILED', extra={'key_id': 500, 'ext_params': json.dumps({'sound': 'checkPoint.wav', 'error': str(e)})})
...
def load_image(name, sx=-1, sy=-1, colorkey=None):
    ...
    try:
        ...
        img = img.convert()
        logger.debug('IMAGE_LOAD_SUCCESS', extra={'key_id': 1, 'ext_params': json.dumps({'file': fullname})})
    except Exception as e:
        logger.error('IMAGE_LOAD_FAILED', extra={'key_id': 500, 'ext_params': json.dumps({'file': fullname, 'error': str(e)})})
        raise
...
def load_sprite_sheet(s_name, namex, namey, scx=-1, scy=-1, c_key=None):
    ...
    try:
        ...
        sh = sh.convert()
        logger.debug('SPRITESHEET_LOAD_SUCCESS', extra={'key_id': 1, 'ext_params': json.dumps({'file': fullname})})
    except Exception as e:
        logger.error('SPRITESHEET_LOAD_FAILED', extra={'key_id': 500, 'ext_params': json.dumps({'file': fullname, 'error': str(e)})})
        raise
    ...
    def update(self):
        ...
        if not self.dead and self.counter % 7 == 6 and self.blinking == False:
            ...
            if self.score % 100 == 0 and self.score != 0:
                ...
                # Лог: +100 очков (чекпоинт)
                logger.info('CHECKPOINT', extra={'key_id': 103, 'ext_params': json.dumps({'score': self.score})})
        self.counter += 1
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE or event.key == pygame.K_UP:
                        ...
                        # Лог: прыжок на стартовом экране
                        logger.info('INTRO_JUMP', extra={'key_id': 101, 'ext_params': json.dumps({'y_pos': ado_dino.rect.bottom})})
        ado_dino.update()
        if pygame.display.get_surface() != None:
...
def gameplay():
    ...
    # Лог: игра начата
    logger.info('GAMEPLAY_START', extra={'key_id': 100, 'ext_params': json.dumps({'game_speed': gp})})
        ...
        while not g_Over:
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_Dino.rect.bottom == int(0.98 * height_screen):
                                ...
                                # Лог: прыжок
                                logger.info('JUMP', extra={'key_id': 101, 'ext_params': json.dumps({'y_pos': gamer_Dino.rect.bottom, 'score': gamer_Dino.score})})
            ...
            for c in cactusan:
                ...
                if pygame.sprite.collide_mask(gamer_Dino, c):
                    ...
                    if pygame.mixer.get_init() != None:
                        die_sound.play()
                    # Лог: столкновение с кактусом
                    logger.warning('COLLISION_DEATH', extra={'key_id': 202, 'ext_params': json.dumps({'obstacle': 'cactus', 'score': gamer_Dino.score})})
            for p in smallBird:
                p.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_Dino, p):
                    ...
                    if pygame.mixer.get_init() != None:
                        die_sound.play()
                    # Лог: столкновение с птицей
                    logger.warning('COLLISION_DEATH', extra={'key_id': 202, 'ext_params': json.dumps({'obstacle': 'bird', 'score': gamer_Dino.score})})
            if len(cactusan) < 2:
                if len(cactusan) == 0:
                    last_end_obs.empty()
            ...
            if gamer_Dino.dead:
                ...
                if gamer_Dino.score > highest_scores:
                    ...
                    # Лог: новый рекорд
                    logger.info('NEW_HIGH_SCORE', extra={'key_id': 104, 'ext_params': json.dumps({'new_high_score': highest_scores, 'old_high_score': highest_scores - gamer_Dino.score})})
            if counter % 700 == 699:
                new_grnd.speed -= 1
                gp += 1
                # Лог: увеличение сложности
                logger.info('DIFFICULTY_INCREASE', extra={'key_id': 102, 'ext_params': json.dumps({'new_game_speed': gp, 'new_ground_speed': new_grnd.speed})})
            counter += 1
        if g_exit:
            break
        while g_Over:
            ...
            time_clock.tick(FPS)
    # Лог: конец игры
    logger.info('GAMEPLAY_END', extra={'key_id': 199, 'ext_params': json.dumps({'final_score': gamer_Dino.score, 'final_high_score': highest_scores})})
    pygame.quit()
    quit()
